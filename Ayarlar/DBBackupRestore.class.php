...
public function __construct() {
	...
	try {
		$this->baglan = new PDO("mysql:host={$host};dbname={$dbname}",$username,$password,
...
public function Disa_Aktar($yol,$zip,$drop,$vtip){
		global $dbname;
		echo "veritabanı için yedek alma işlemi başladı....<br><br>";
		$this->sonuc = "-- "" veritabanı için sql yedeği.\n\n";
		$this->TabloCek($vtip);
		$this->Olustur($drop);

        $date=date('d-m-Y_H-i-s');
    if ($zip == true){
        $dosya = $yol . "Yedek-$dbname-$date.sql.zip";
        $zip = new ZipArchive();
        if ($zip->open($dosya, ZIPARCHIVE::CREATE) !== TRUE) {
            exit("Zip Dosya açılamadı <$dosya>\n");
        }
        $zip->addFromString("Yedek-$dbname-$date.sql", $this->sonuc);
        $zip->close();
	} else {
        $dosya = $yol . "Yedek-$dbname-$date.sql";
        $fp = fopen($dosya, 'w');
...
private function TabloCek($vtip){
		try {
			...
			ob_flush();
			flush();
			    echo "<strong>" . $tablo[0] . "</strong> tablosu >>>> TAMAM<br><br>";
				$i++;
			}
			unset($sql);
			unset($sonuc);
