...
<head>
	...
	<title>Alınan Çekler - <?= $ayar["siteadi"] ?> - <?= $ayar["siteslogan"] ?></title>
	<?php
...
<div class="col-sm-9 col-md-9">
									...
									<h4 class="card-title">ALINAN ÇEK LİSTESİ</h4>
								</div>
...
<button id="Kaydet" name="kaydet" type="submit" class="btn btn-success btn-raised btn-block" onclick="window.location.href='AlinanCekEkle.php'">
Çek Ekle</button>
<?php }?>
	<br>
...
<th>İşlem</th> 
        ...
        <tbody id="Urunlistesi">      
                      <?php                                                                                                               $sorgu = $baglanti->query("
                                                                                SELECT * FROM CekSenet where islem='alinan' and ceksenet='cek'")->fetchAll();
...
<td>
 ...
 if ($kbilgi["alinancekduzenle"]==1) {?>															
										<button  type="button" data-toggle="tooltip" title="Düzenle" class="btn btn-icon btn-round btn-warning" data-original-title="Düzenle" onclick="window.location.href='AlinanCekDuzenle.php?id=<?= $sonuc["id"] ?>'">
