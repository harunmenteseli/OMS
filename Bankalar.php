...
<head>
	...
	<title>Bankalar - <?= $ayar["siteadi"] ?> - <?= $ayar["siteslogan"] ?></title>
	<?php
...
<div class="card">
								<div class="card-header">
									<h4 class="card-title">Banka Listesi</h4>
								</div>
								<div class="card-body ">
	<?php 
 if ($kbilgi["bankaekle"]==1) {?>
<button id="Kaydet" name="kaydet" type="submit" class="btn btn-success btn-raised btn-block" onclick="window.location.href='BankaEkle.php'">
Banka Ekle</button>
<?php }?>
<br>
...
<?php $sorgu = $baglanti->query("SELECT * FROM Bankalar")->fetchAll();
foreach ($sorgu as $sonuc) {
?> 
<tr>

<td><?= $sonuc['bankaadi']?></td>
<td class="text-right">
<?php $parabirimleri = $baglanti->query("SELECT * FROM ParaBirimleri")->fetchAll();
foreach($parabirimleri as $parabirimi){
?>
<?php
$kasagiristopla=$baglanti->prepare("SELECT SUM(tutar) AS girisler FROM BankaGiris where islem='bankagirdi' and bankaadi='$sonuc[id]' and parabirimi='$parabirimi[id]'");
...
?>
<?php
$kasacikistopla=$baglanti->prepare("SELECT SUM(tutar) AS cikislar FROM BankaGiris where islem='bankacikti' and bankaadi='$sonuc[id]' and parabirimi='$parabirimi[id]'");
...
<?php
$kalangiristopla=$baglanti->prepare("SELECT SUM(tutar) AS girisler FROM BankaGiris where islem='bankagirdi' and bankaadi='$sonuc[id]' and parabirimi='$parabirimi[id]'");
$kalangiristopla->execute();
$kalangirisyaz= $kalangiristopla->fetch();
$kalangiristoplam=$kalangirisyaz['girisler'];
?>
<?php
$kalancikistopla=$baglanti->prepare("SELECT SUM(tutar) AS cikislar FROM BankaGiris where islem='bankacikti' and bankaadi='$sonuc[id]' and parabirimi='$parabirimi[id]'");
...
<td>
														<div class="form-button-action">
															<button type="button" data-toggle="tooltip" title="İncele" class="btn btn-icon btn-round btn-info" data-original-title="İncele" onclick="window.location.href='BankaGirisDetay.php?id=<?= $sonuc["id"] ?>'">
																<i class="fa fa-search"></i>
															</button>
															
			<?php 
 if ($kbilgi["bankaduzenle"]==1) {?>															
										<button  type="button" data-toggle="tooltip" title="Düzenle" class="btn btn-icon btn-round btn-warning" data-original-title="Düzenle" onclick="window.location.href='BankaDuzenle.php?id=<?= $sonuc["id"] ?>'">
