...
<head>
	...
	<title>Mesaj Düzenle - <?= $ayar["siteadi"] ?> - <?= $ayar["siteslogan"] ?></title>
	<?php
...
<input type="text" name="id" class="form-control" placeholder="id" readonly="readonly" value="<?= $sonuc["id"] ?>" hidden="hidden">
...
<?php $giden = $baglanti->query("SELECT * FROM Kullanicilar where id=".$sonuc["kime"])->fetchAll(PDO::FETCH_ASSOC);
if(count($giden)>0){ ?>
<?php
foreach($giden as $gidenim){
?>
 <option value="<?= $gidenim['id']?>"><?= $gidenim['ad']?> <?= $gidenim['soyad']?></option>
                                                    <?php }?>
       <?php } ?>
 ...
 <input type="text" id="birimadi" class="form-control" placeholder="Mesaj Başlığı" name="baslik" value="<?= $sonuc['baslik']?>">
                                                  </div>
                                            </div>
                                            <!--/span-->
                                            <div class="col-md-12">
   ...
   <label class="control-label">Mesaj İçeriği</label>
           <textarea class="form-control" id="mesaj" rows="5" name="mesaj"><?= $sonuc['mesaj']?></textarea>            
                                                  </div>
                                            </div>
                                            <!--/span-->
									...
									<input type="hidden" class="form-control" name="mesajduzenle">
