...
<head>
	...
	<title>Alinan Senetler - <?= $ayar["siteadi"] ?> - <?= $ayar["siteslogan"] ?></title>
	<?php
...
<div class="col-sm-9 col-md-9">
									...
									<h4 class="card-title">ALINAN SENET LİSTESİ</h4>
								</div>
...
<button id="Kaydet" name="kaydet" type="submit" class="btn btn-success btn-raised btn-block" onclick="window.location.href='AlinanSenetEkle.php'">
Senet Ekle</button>
<?php }?>
	<br>
...
<th>İşlem</th> 
        ...
        <tbody id="Urunlistesi">      
                      <?php                                                                                                               $sorgu = $baglanti->query("
                                                                                SELECT * FROM CekSenet where islem='alinan' and ceksenet='senet'")->fetchAll();
...
<td class="tutar text-success"><?= number_format($sonuc['tutar'], 2, ',', '.');?> <?php $parabirimi = $baglanti->query("SELECT * FROM ParaBirimleri where id=".$carisi["parabirimi"])->fetchAll(PDO::FETCH_ASSOC);
...
?>
<?= number_format($kurgenelsonuc, 2, ',', '.'); ?> TL</td>
<td class="barkod">
     <?php 
...
<td>
 ...
 if ($kbilgi["alinansenetduzenle"]==1) {?>															
										<button  type="button" data-toggle="tooltip" title="Düzenle" class="btn btn-icon btn-round btn-warning" data-original-title="Düzenle" onclick="window.location.href='AlinanSenetDuzenle.php?id=<?= $sonuc["id"] ?>'">
  ...
  <div class="modal-dialog modal-lg">
                      <div class="modal-content bg-<?= $tema['arkaplan']?>">
                        <div class="modal-header bg-<?= $tema['tablorenk']?>">
                          <h4 class="modal-title" id="myModalLabel8">SENET DETAYI</h4>
        ...
        $(document).ready(function(){
           ...
           var productId = $(this).data('id'); 	
				...
				swal({
                    ...
                    data: 'alinansenetsil='+productId, // Formdaki tüm verileri al
