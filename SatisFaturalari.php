...
<head>
	...
	<title>Satış Faturaları- <?= $ayar["siteadi"] ?> - <?= $ayar["siteslogan"] ?></title>
	<?php
...
<div class="col-sm-9 col-md-9">
									...
									<h4 class="card-title">SATIŞ FATURALARI</h4>
								</div>
...
<th>İşlem</th> 
     ...
     <?php                                                                                                               $sorgu = $baglanti->query("
                                                                                SELECT * FROM SatFatBilgi")->fetchAll();
...
<?= number_format($sonuc['geneltoplam'], 4, ',', '.');?> <?php $parabirimi = $baglanti->query("SELECT * FROM ParaBirimleri where id=".$cari["parabirimi"])->fetchAll(PDO::FETCH_ASSOC);
...
?>
<?= number_format($kurgenelsonuc, 4, ',', '.'); ?> TL
</td>       
<td class="barkod">
...
<td>
 ...
 if ($kbilgi["satisfaturaduzenle"]==1) {?>															
															<button  type="button" data-toggle="tooltip" title="Düzenle" class="btn btn-icon btn-round btn-warning" data-original-title="Düzenle" onclick="window.location.href='SatisFaturasiDuzenle.php?id=<?= $sonuc["id"] ?>'">
        ...
        $(document).ready(function(){
           ...
           var productId = $(this).data('id'); 	
				...
				swal({
                    ...
                    data: 'satissil='+productId, // Formdaki tüm verileri al
